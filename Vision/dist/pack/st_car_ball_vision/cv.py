...
# -*- coding: utf-8 -*-
"""
MaixCAM2 - 电赛 H 题滚球位置检测（fp58：对侧机位与稳健重捕获）
...
from maix import camera, display, image, app, time, uart, pinmap, err
import cv2
import numpy as np
import math
import os
...
MARKER_MAX_JUMP_PX = 22.0
MARKER_SPAN_CHANGE_MAX = 0.16
...
BALL_CENTER_Y_TOLERANCE_DIAMETER = 1.00
BALL_ROI_EXTENSION_RATIO = 0.060
BALL_VALID_POSITION_LIMIT_CM = 11.5
# Paper end-stops added on 2026-07-31 keep the free ball visible slightly beyond
# the +/-100 mm calibration strips.  Search the same 6% span extension already
# present in the ball ROI; appearance, four-frame confirmation and the explicit
# +/-115 mm physical coordinate limit remain the acquisition safety gates.
BALL_ACQUIRE_EDGE_GUARD_RATIO = -0.060
...
BALL_DARK_ACQUIRE_SCORE = 3.20
BALL_ACQUIRE_SCORE_MARGIN = 0.40
BALL_TRACK_SCORE = 3.00
BALL_PRIMARY_MAX_JUMP_PX = 60.0
BALL_LOCAL_SEARCH_HALF_WIDTH_PX = 40.0
BALL_OPTICAL_FLOW_MAX_JUMP_PX = 40.0
BALL_TRACK_JUMP_GROWTH_PX = 3.0
BALL_TRACK_MOTION_GAIN = 10.0
BALL_TRACK_MAX_EXTRA_JUMP_PX = 60.0
BALL_FALLBACK_MAX_JUMP_PX = 12.0
BALL_HOLD_FRAMES = 30
...
# At roughly 60 FPS, even the UART-side 700 mm/s physical-speed ceiling moves
# the ball by less than about 14 px per frame in the present marker span.
# A wider gate allowed unrelated tube highlights to form a fake acquisition
# whose reported position wandered when no ball was visible.
...
BALL_REACQUIRE_EDGE_ZONE_RATIO = 0.22
BALL_REACQUIRE_CORRIDOR_RATIO = 0.32
# The same-side corridor is useful immediately after an end-cap occlusion, but
# it must not become permanent.  A bounded recovery tilt can carry the unseen
# free ball to the opposite end; after roughly three seconds at 60 FPS, fall
# back to the robust global acquisition gates.
BALL_REACQUIRE_CORRIDOR_MAX_FRAMES = 180
BALL_DARK_VALUE_MAX = 105
BALL_DARK_VALUE_MIN = 15
BALL_CORE_MASK_VALUE_MAX = 155
BALL_MIN_DARK_FRACTION = 0.22
BALL_MIN_LOCAL_CONTRAST = 7.5
BALL_TRACK_MIN_DARK_FRACTION = 0.10
BALL_TRACK_MIN_LOCAL_CONTRAST = -25.0
BALL_LOW_CONTRAST_CUTOFF = 10.0
BALL_LOW_CONTRAST_MIN_DARK_FRACTION = 0.40
BALL_SPECULAR_MIN_DARK_FRACTION = 0.18
BALL_SPECULAR_MIN_GRAY_STD = 30.0

VISION_BUILD = "2026-07-31-fp60-visible-paper-end-stops"

# 最终坐标 Alpha-Beta 滤波。
POSITION_ALPHA = 0.72
POSITION_BETA = 0.08
POSITION_RESET_RESIDUAL_CM = 2.5
...
# 调试画面。MaixVision 预览会占用明显传输时间；检测不依赖预览。
                    ...
                    if tube_axis is not None:
                        ...
                        if self._validate_marker_x(
                            ...
                            state = {
                                ...
                                "marker_x": tuple(float(v) for v in marker_x),
                                "axis_y": float(axis_y),
...
class BallDetector:
    ...
    def _tracking_window(self):
        """Return motion-predicted center and a bounded tracking jump gate."""
        if self.lost_frames <= 0:
            return float(self.last_x), BALL_PRIMARY_MAX_JUMP_PX
        frames_ahead = float(self.lost_frames + 1)
        predicted_x = float(self.last_x) + self.motion_x * frames_ahead
        motion_extra = min(
            BALL_TRACK_MAX_EXTRA_JUMP_PX,
            abs(self.motion_x) * BALL_TRACK_MOTION_GAIN * frames_ahead,
        )
        jump_limit = (
    ...
    def _local_specular_track(
        ...
        if not (x_left + edge_guard <= self.last_x <= x_right - edge_guard):
            return None
        predicted_x, _ = self._tracking_window()
        search_left = max(
            x_left,
    ...
    def _optical_flow_track(
        ...
        if abs(tracked_x - self.last_x) > BALL_OPTICAL_FLOW_MAX_JUMP_PX:
            return None
        axis_origin_x = rotated.shape[1] * 0.5
        local_axis_y = (
    ...
    def detect(self, marker_state):
        ...
        for index in range(1, dark_count):
            ...
            dark_width_max = (2.80 if tracking else 2.30) * expected_diameter
            dark_height_min = (0.24 if tracking else 0.38) * expected_diameter
            dark_height_max = (2.30 if tracking else 2.40) * expected_diameter
            dark_aspect_max = 4.00 if tracking else 2.00
            dark_fill_min = 0.18 if tracking else 0.30
            dark_area_max = (2.80 if tracking else 3.00) * expected_diameter ** 2
            if not (0.38 * expected_diameter <= w <= dark_width_max):
                continue
            if not (dark_height_min <= h <= dark_height_max):
                continue
            if not (
                0.10 * expected_diameter * expected_diameter
                <= area
                <= dark_area_max
            ):
                continue
            aspect = w / float(max(1, h))
            fill = area / float(max(1, w * h))
            if not (0.58 <= aspect <= dark_aspect_max and
                    dark_fill_min <= fill <= 0.92):
                continue
            ...
            self.debug_appearance.append(
                (round(global_x, 1), round(global_y, 1), w, h, area,
                 round(contrast, 1), round(dark_fraction, 2), "D")
            )
            if not self._appearance_valid(
                    contrast, dark_fraction, gray_std, tracking):
                continue
        ...
        if tracking:
            predicted_x, tracking_jump_limit = self._tracking_window()
            ranked_candidates = [
            ...
            if self.reacquire_side < 0:
                edge_limit = plus_x + span * BALL_REACQUIRE_CORRIDOR_RATIO
                ranked_candidates = [
                    item for item in ranked_candidates
                    if float(item["x"]) <= edge_limit
                ]
            elif self.reacquire_side > 0:
                edge_limit = minus_x - span * BALL_REACQUIRE_CORRIDOR_RATIO
                ranked_candidates = [
                    item for item in ranked_candidates
                    if float(item["x"]) >= edge_limit
                ]
        ...
        if tracking:
            ...
            threshold = BALL_DARK_ACQUIRE_SCORE
        else:
        ...
        if best is None or best["score"] < threshold:
            ...
            if not tracking and hough_candidate is not None:
                ...
                wrong_reacquire_corridor = (
                    (self.reacquire_side < 0 and
                     hough_x > plus_x + span * BALL_REACQUIRE_CORRIDOR_RATIO) or
                    (self.reacquire_side > 0 and
                     hough_x < minus_x - span * BALL_REACQUIRE_CORRIDOR_RATIO)
                )
                fixed_hardware = (
                ...
                if (hough_candidate["score"] < BALL_HOUGH_ACQUIRE_SCORE or
                        ...
                        wrong_reacquire_corridor or fixed_hardware):
        ...
        if best is None:
            ...
            if self.lost_frames > BALL_HOLD_FRAMES:
                edge_zone = span * BALL_REACQUIRE_EDGE_ZONE_RATIO
                if self.last_x is not None:
                    if self.last_x <= plus_x + edge_zone:
                        self.reacquire_side = -1
                    elif self.last_x >= minus_x - edge_zone:
                        self.reacquire_side = 1
                self.last_x = None
                self.last_y = None
                if (
                    self.lost_frames >
                    BALL_HOLD_FRAMES + BALL_REACQUIRE_CORRIDOR_MAX_FRAMES
                ):
                    self.reacquire_side = 0
            return None
        ...
        if not tracking:
            if (self.pending_x is not None and
                ...
                self.pending_count = 1
            if self.pending_count < BALL_ACQUIRE_CONFIRM_FRAMES:
                return None
        ...
        if self.last_x is not None:
            instant_motion_x = (
                ...
                float(self.lost_frames + 1)
            )
            instant_motion_x = clamp(instant_motion_x, -25.0, 25.0)
            self.motion_x = 0.65 * self.motion_x + 0.35 * instant_motion_x
        else:
            self.motion_x = 0.0
...
def main():
    ...
    serial = init_uart()
    print("VISION_BUILD", VISION_BUILD)
    marker_tracker = PlasticMarkerTracker()
    ball_detector = BallDetector()
    position_median = RollingMedianFilter(3)
    ...
    while not app.need_exit():
        ...
        if marker_state is not None:
            ball = ball_detector.detect(marker_state)
            if ball is not None:
                plus_x, zero_x, minus_x = marker_state["marker_x"]
                ...
                if (mapped is not None and
                    ...
                    # The ball disappears behind either end cap outside the
                    # calibrated tube span.  Never let an out-of-range fixed
                    # structure seed or maintain the local trackers.
                    ball_detector.reset()
        else:
            ball_detector.reset()
            position_median.reset()
        ...
        if serial is not None and elapsed_ms(now_ms, last_uart_ms) >= UART_SEND_PERIOD_MS:
            ...
            try:
                if position_valid:
                    serial.write_str(
                        "$B,{:d},1\r\n".format(int(round(filtered_cm * 10.0)))
                    )
                else:
                    serial.write_str("$B,0,0\r\n")
            except Exception as exception:
