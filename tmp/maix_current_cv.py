...
BALL_REPORTED_POSITION_LIMIT_CM = 12.5
...
BALL_ENDPOINT_DARK_ACQUIRE_SCORE = 2.35
...
BALL_MARKER_ARTIFACT_RADIUS_DIAMETERS = 1.15
...
BALL_TRACK_MIN_LOCAL_CONTRAST = 2.0
...
BACKGROUND_ACQUIRE_MIN_FRACTION = 0.18
BACKGROUND_ENDPOINT_ACQUIRE_MIN_FRACTION = 0.24
BACKGROUND_TRACK_MIN_FRACTION = 0.12
BACKGROUND_TRACK_GLOBAL_MIN_FRACTION = 0.35
BACKGROUND_SCORE_GAIN = 2.50

VISION_BUILD = "2026-08-01-velocity-uart-v28-fast-state"
...
class BallDetector:
    ...
    def detect(self, marker_state, now_ms):
        ...
        for index in range(1, dark_count):
            ...
            dark_width_max = ((4.20 if endpoint_partial else 2.80)
                              if tracking else
                              (4.20 if endpoint_partial else 2.30)) * expected_diameter
            dark_height_min = (
            ...
            dark_area_max = ((6.20 if endpoint_partial else 2.80)
                             if tracking else
                             (6.20 if endpoint_partial else 3.00)) * expected_diameter ** 2
            dark_width_min = (
            ...
            endpoint_merged = endpoint_partial and w > 1.65 * expected_diameter
            endpoint_compact = (
            ...
            if (not endpoint_sliver and not self._appearance_valid(
                    contrast, dark_fraction, gray_std, tracking)):
                continue
        ...
        if tracking:
            candidates = [
                ...
                if (not undersized_marker_artifact(item) and
        ...
        allow_fallbacks = self.background is None
        ...
        if tracking:
            ...
            out_of_window_background = (
                not ranked_candidates and any(
                    float(item.get("background_fraction", 0.0)) >=
                    BACKGROUND_TRACK_GLOBAL_MIN_FRACTION
                    for item in candidates
                )
            )
        else:
        ...
        if marker_state is not None:
            ...
            if ball is not None:
                ...
                if (mapped is not None and
                    ...
                    # Detect the full physical pocket, while saturating the
                    # UART coordinate inside its verified +/-125 mm range.
                    raw_cm = clamp(
                        float(mapped),
