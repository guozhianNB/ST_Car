...
| 模块 | 责任 | 禁止事项 |
|---|---|---|
| `motor` | 三路 TB6612 方向、PWM、制动/滑行、STBY；运行时限幅优先于最低 PWM 补偿 | 不读取编码器，不运行控制器 |
...
| `bench_debug` | 可编译移除的平衡台架命令、P60 原始 PWM 限时脉冲、运行时标定、扩展遥测和五秒静态滚球序列 | 不控制底盘；未显式开启时不得使能电机 |
...
台架 `limit pwm` 是实际电机命令的硬上限。SA100 的 TIM15 捕获使用 0.1 us 计数，以避免约 1.15 ms PWM 周期下 1 us 量化造成约 0.31° 的角度台阶；`per/high` 遥测字段仍按 us 输出。角度反馈在捕获侧使用集中配置的一阶低通，角度内环在误差进入集中配置的停止死区后清零 PID 并制动。摆杆角度闭环使用受限的连续原始 PWM，不强制最低 PWM 补偿；否则很小 PID 输出会变成固定幅值正反脉冲并形成极限环。每次停止或换向后还必须保持集中配置的制动时间，禁止机构惯性未消失时立即反打。`APP_MOTOR_BEAM_MIN_PWM` 保留为开环启动特性参数。
...
4. 人工刻线定义的 P60 安全中点、两侧计数边界和堵转阈值；当前不使用微动开关或自动寻零；
