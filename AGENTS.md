...
- `VERIFIED（用户/指导教师决策，2026-07-31）`：正式滚球改用视觉单环 PID，反馈仅为 MaixCAM2 的球位置和由真实视觉帧导出的球速，PID 直接输出 P60 电机 PWM；SA100 不再进入滚球控制链、启动门禁或故障判断。
- 三个电机复位、待机和故障时必须零输出；两路 TB6612 `STBY` 默认保持低。
- `VERIFIED（用户授权，2026-07-31）`：当前机构具有独立保护且可用行程充足，正式视觉单环不依赖 P60 中点或软件位置边界；P60 编码器仅用于堵转检测与调试，急停和复位/待机零输出仍保留。
...
4. P60 脱球/低 PWM，确认编码器方向、堵转特征和急停；
5. 固定小车完成视觉 `-100,0,+100 mm`、延迟和丢球测试；
6. 空管用视觉直驱链的受限手动 PWM 确认 P60 正反方向、制动和急停；
7. 小车静止、最低 PWM 上限完成视觉单环球位置控制；
8. 完成要求3并验证任意位置保持；
...
- 不得让 CubeMX 恢复旧的 TIM4 双 PWM、删除 UART4 RX DMA、把 STBY 默认拉高或重新占用 SWD。TIM15/SA100 已退出正式控制链，后续生成代码不得把它重新接回滚球控制。
- 手工业务逻辑不得重新堆回 `main.c` 或中断函数。ISR 只做捕获、快照、时间戳和事件标志。

## 7. 代码边界与并发规则

- `motor` 只负责 TB6612；`encoder` 只负责计数与速度；`vision_uart`、`line_sensor` 只生产传感器数据。SA100 已退出正式构建与控制链。
